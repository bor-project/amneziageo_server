...
что на нём есть: система и ядро, ждущие обновления, модуль `amneziawg`, память и диск, Docker, пересылка и BBR,
ufw, занятые порты с именами служб, интерфейсы AmneziaWG и WireGuard, сертификаты Let's Encrypt. Потом задаёт все
вопросы сразу, с ответами по умолчанию из осмотра, показывает план и после согласия:
...
- скачивает пакет последнего релиза, сверяет его с ключом, которым подписаны релизы, заводит первого
  администратора с временным паролем и запускает панель (шаги 5 и 6);
- выпускает сертификат Let's Encrypt на имя сервера или берёт уже выпущенный на это имя и открывает панель на всех
  адресах хоста (шаг 7);
- заводит первый интерфейс и клиента: конфигурация клиента ложится в `/root`, по желанию и QR-кодом (шаг 9);
- открывает нужные порты в ufw или включает его, оставляя открытыми ssh и то, что уже слушает (шаг 10);
- проверяет панель, сертификат и интерфейс (шаг 8) и печатает адрес панели, логин и временный пароль.
...
На вопрос `Install as` ответьте `1` (пакет), на `Channel` - `1` (обычные релизы) или `2` (беты). Ответы можно
...
- создаёт `/var/lib/amneziageo-server` (база) и `/etc/amneziageo-server` с заготовкой `server.env` (все строки
  закомментированы);
- ставит юниты `amneziageo-server.service` и `amneziageo-proxy@.service` (фронты WebSocket прокси); программа
  `wstunnel` лежит в самом релизе, свой `/usr/local/bin/wstunnel` хоста пакет не трогает;
...
Поэтому вариант Б выбирают до первого `systemctl start`. Если служба уже работала,
...
С ufw панель сама добавляет свои правила, когда включено «Открыть порт в брандмауэре» у интерфейса или «держать
порт открытым» у панели и подписок (пункт 23 меню `amneziageo-server`), а включать и выключать ufw не берётся.
Закрытый порт панели и подписок вкладки «Сервер» и «Подписки» называют под полем порта. Руками для интерфейса `awg0` на порту 51820:
...
| `/var/lib/amneziageo-server/backup` | копии базы перед обновлениями и восстановлением из бэкапа |
...
| `/opt/amneziageo-server/current/wstunnel` | фронт WebSocket прокси |
