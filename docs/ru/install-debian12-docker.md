...
базу и дальше читает оттуда. Поэтому вариант Б выбирают до первого `docker compose up -d`. Если контейнер уже
...
и в браузере `http://localhost:8443/`.
...
В образе нет ufw, поэтому панель кладёт свои таблицы nftables, а галочка «Открыть порт в брандмауэре» правила ufw
не пишет. Если ufw на хосте включён, его `drop` сильнее, и порты открываются руками. Для интерфейса `awg0` на
порту 51820:
