...
образ, удаление, сертификат Let's Encrypt, правила ufw, BBR, пересылка), отвечают, что из контейнера до хоста не
достать. Команды утилиты идут через то же имя: `docker compose exec panel amneziageo-server ...` из шагов ниже
работают как прежде.
