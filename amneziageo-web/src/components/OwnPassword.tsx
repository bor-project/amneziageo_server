...
import { card, field, label, primary } from "@/components/styles"
...
export function OwnPassword() {
  ...
  return (
    <form onSubmit={submit} className={`mt-4 p-4 ${card}`}>
      <div className="text-sm font-medium text-ink">{t("password.title")}</div>
      ...
      {mismatch && <div className="mt-3 text-sm text-alarm">{t("password.mismatch")}</div>}
      {error && <div className="mt-3 rounded bg-alarm-soft px-3 py-2 text-sm text-alarm">{t(error)}</div>}
      {done && <div className="mt-3 text-sm text-brand-ink">{t("password.done")}</div>}

      <button type="submit" disabled={busy || !ready} className={`mt-4 ${primary}`}>
