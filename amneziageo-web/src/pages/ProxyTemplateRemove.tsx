...
import { useProxyTemplates, useRemoveProxyTemplate } from "@/api/proxyTemplates"
import { useTail } from "@/components/crumbs"
import { card, danger, secondary } from "@/components/styles"
import { useText } from "@/i18n"

const list = "/connections/templates/proxies"

export function ProxyTemplateRemove() {
  const t = useText()
  const navigate = useNavigate()
  const { templateId } = useParams()
  const templates = useProxyTemplates()
  const remove = useRemoveProxyTemplate()
    ...
    <div className="mt-4 flex max-w-[35rem] flex-col gap-4">
      ...
      <div className={`border-alarm-line p-4 ${card}`}>
        ...
        <div className="mt-1 text-[13px] text-muted">
          {t("templates.proxiesCount", { count: String(held.proxies) })}
        </div>
      </div>
